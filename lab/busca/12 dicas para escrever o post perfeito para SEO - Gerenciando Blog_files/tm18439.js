var nvg18439 = new function(){
  this.w=window;
  this.d=document;
  this.version= 8; 
  this.url=this.dom=this.usr=this.ctr=this.wct=this.wcc=this.qry=this.nvgc=this.nvgt=this.crl=this.hca=0;
  this.wst=1;/* want string*/
  this.syn=this.val=this.nvg='';
  this.acc = 18439;
  this.hca = true;
  this.tmc = 'nvg18439.cokCustom=function(a){fmtSegments="gender age education city region country interest product class marital income".split(" ");if(this.control!=this.datestr()){cokStr=a+"_"+this.datestr();for(nvg_x=0;fmtSegments[nvg_x];nvg_x++){segl=this.getSegment(fmtSegments[nvg_x]);if(0<=segl.search("-")){segl=segl.split("-");tmpl=[];for(nvg_y=0;10>=nvg_y;nvg_y++)segl[nvg_y]&&(tmpl[nvg_y]=segl[nvg_y]);segl=tmpl.join(",")}cokStr+="_"+segl}this.setCookie(this.coknm,cokStr)}};var nvg=nvg18439;';

  this.addons = {'nvgPromo': [{'acc': '18439', 'product': '', 'name': 'Potencialize seu blog', 'title': 'Potencialize seu blog', 'url': 'http://www.gerenciandoblog.com.br/p/contrate-servicos-para-o-seu-blog.html?nvgc=506', 'custom': '', 'cluster': '', 'color': 'blue', 'btnTxt': 'Clique Aqui', 'position': 'bottom', 'icon': '', 'id': '172', 'nvgc': '506', 'desc': 'Voc\xea n\xe3o est\xe1 conseguindo realizar configura\xe7\xf5es em seu blog? Confira o que preparamos para te ajudar'}]}
  this.loadAddOns=function(){for(var nvg_add in this.addons)this.include('//tag.navdmp.com/'+nvg_add+'loader.js');}
  this.account && (this.acc=this.account);
  this.ckn='nav'+this.acc;
  this.ser = ['navdmp.com','navdmp.com'];/* server */
  this.seg = "gender age education marital income city region country connection brand product interest career cluster prolook custom".split(" ");/* segment positions */
  ...
  this.preLoad=function(){
    ...
    if(typeof(this.w.localStorage)=="object"){
      lsv=this.w.localStorage.getItem(this.ckn);
  ...
  this.load=function(){
    ...
    if(!this.usr || this.tmg || this.ctr!=this.datestr() || this.getParameter('navegg_debug')=='1'){
      var url='/usr?v='+this.version;
      url+='&acc='+this.acc;
      if(!this.ctr || (this.ctr != this.datestr()) )url+='&upd=1';
      if(this.usr)url+='&id='+this.usr;
      else url+='&new=1';
    ...
    d = new Date();
      if(this.nvgt){
        ...
        if(this.nvgt[2]!=this.nvgc){
          time=(d.getTime() - parseInt(this.nvgt[0]));
  ...
  this.getParameter=function(fld){
    if(this.qry==0){
      this.qry = {};
      prmstr = this.w.location.search.substr(1);
      prmarr = prmstr.split ("&");
  ...
  if(this.crl){
    ...
    for(nvg_arr_pos=0;nvg_arr_pos<this.crl.length;nvg_arr_pos++){
      ...
      for(nvg_pos=0;nvg_pos<nvg_rules.length;nvg_pos++)
        if(window.location.href.search(nvg_rules[nvg_pos])>=0)
          return nvg_camId;
    }
  }
  return 0;
...
function nvgGetSegment(f){return nvg18439.getSegment(f);};
function ltgc(s){return nvg18439.getSegment(s);};
nvgAddOns = nvg18439.addons;
